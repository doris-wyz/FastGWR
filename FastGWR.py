...
def local_fit(i,bw,final=False,adaptive = True):

    #adaptive bisquare
    if adaptive:
        dist = cdist([coords[i]],coords).reshape(-1)
        maxd = np.partition(dist, int(bw)-1)[int(bw)-1]
        zs = dist/maxd
        zs[zs>=1] = 1
...
def mpi_gwr_fit(bw,final=False,fout='./fastGWRResults.csv'):
    #Need Betas
    if final:
        sub_Betas = np.empty((x_chunk.shape[0],2*k+5), dtype=np.float64)
        #print(x_chunk.shape[0])
        pos = 0
        for i in x_chunk:
            sub_Betas[pos] = local_fit(i,bw,True)
            pos+=1
        ...
        if rank ==0:
            ...
            llf = -np.log(RSS)*n/2 - (1+np.log(np.pi/n*2))*n/2
            aicc = -2*llf + 2.0*n*(trS + 1.0)/(n-trS-2.0)
            data[:,-k:] = np.sqrt(data[:,-k:]*sigma2_v1v2)
...
if __name__ == "__main__":
    ...
    if bw is None:
        gwr_func = lambda bw: mpi_gwr_fit(bw)
        bw = golden_section(int(45), n, 0.38197, gwr_func)
        mpi_gwr_fit(bw,final=True,fout=fout)
    ...
    if rank ==0:
        print("Total Time Elapsed:",max(wt),"seconds")
